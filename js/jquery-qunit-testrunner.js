...
$.extend(window, {
	...
	QUnit: {
		equiv: equiv,
		ok: ok,
		done: function(result, failures, total, resultsURL){
        if (resultsURL) {
          $.ajax({ type     : 'GET',
                   url      : resultsURL,
                   data     : "result=" + result + "&failures=" + failures + "&total=" + total,
                   dataType : "json"});
        }
		},
...
function runTest() {
	...
	synchronize(function() {
		$('<p id="testresult" class="result"/>').html(['Tests completed in ',
			...
			.join(''))
			.appendTo("body");
		var result = 	config.stats.bad ? "fail" : "pass";
		$("#banner").addClass(result);
		var resultsURL = location.search.slice(1).split("=")[1];
		QUnit.done( result, config.stats.bad, config.stats.all, resultsURL);
	});
}
